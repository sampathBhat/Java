...
public class customProducer {
	...
	public customProducer(String configPath) throws IOException {
		// Read initial configuration
		System.out.println("In the producer constructor ");
		...
		System.out.println("In the producer constructor "+props.toString()+" topic  "+topic+"   watchdir  "+watchDir);
	}
	...
	public static void main(String args[]) {
		...
		try {
			abp = new customProducer(args[0]);
			try {
				abp.start();
			} catch (InterruptedException ex) {
				Logger.getLogger(customProducer.class.getName()).log(Level.SEVERE, null, ex);
			}
